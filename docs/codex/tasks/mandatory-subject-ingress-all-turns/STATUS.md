...
- `M2` 当前仍在收口：
  ...
  - `evidence followup reply` 与 `read_only_preflight` / `force_waiting_input` / `direct_reply_text` 仍需补齐 subject-gated finalize / response-plan 强制路径
...
- 补齐 `evidence_followup` 与 `pre_runtime` host-visible replies 的 finalize / response-plan 强制路径
- 重新跑定向验证，确认 M2 真的可接受
...
- 当前只能宣称：
  ...
  - `M2 Telegram Runtime_V2 Early-Return Closure` 还未完全接受
