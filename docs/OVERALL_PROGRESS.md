...
| 当前唯一 build-first candidate | `active-inference self-model` | 已通过与 `MVS` 相同的 held-out replay gate，并已通过 repo-authored controlled conversation replay bridge；当前仍只到 bounded research evidence，不是 runtime proof |
...
| 距离当前工程目标剩余阶段 | `>= 2（best-case）` | 指 `controlled observation -> selection closeout`，不是“真正主观体验” |
...
- `active-inference self-model` = 已在同一 held-out replay gate 下通过，并已在 repo-authored controlled conversation replay bridge 下继续过线，但仍是 bounded research evidence
...
- `Stage 4 / Milestone 19: Controlled Observation Planning`
...
1. 冻结 repo-authored conversation manifest
   - observation bank 必须继续保持可回放、可审计、可回退
   - family 至少覆盖：
     - `identity_continuity`
     - `decision_conflict`
     - `failure_repair_retry`
2. 冻结 controlled observation runner contract
   - 继续只使用既有：
     - `policy_hint`
     - `response_tendency`
     - `trace_payload`
   - observation bridge 只归一化到既有：
     - `KernelEvent`
     - `external_result`
     - `state snapshot`
...
4. 用 controlled observation planning 决定是否进入下一轮证据升级
   - 若 observation design 能继续保持 bounded contract 与 replayable trace，再实现 first observation runner
...
- controlled integration plan 与 controlled replay bridge 虽已落地，但还没有 controlled observation 级别证据
