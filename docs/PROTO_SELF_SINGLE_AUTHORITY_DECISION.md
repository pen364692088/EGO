...
| `self-model` | `openemotion.self_model/*` | `openemotion.proto_self.self_model` + v1 `SelfModel` state | `OpenEmotion/emotiond/self_model_adapter.py` | `OpenEmotion/emotiond/self_model_mirror.py` | formal owner 是唯一 authority；v1 substrate 保留为 active compute/proposal layer，不再叙述成 authority |
...
| `OpenEmotion/emotiond/self_model_adapter.py` | 降级为 compat/shim | 只保留历史接线兼容与旧 proof/tooling 参考；不是 formal mainline |
...
| `OpenEmotion/emotiond/self_model_mirror.py` | reference-only | read-only mirror / 对照面，不拥有 authority |
...
| `EgoCore/app/openemotion_adapter/proto_self_restore.py` | 候删 | 当前 formal mainline 无 caller；先保留 compat/shim 分类，下一轮再做删除 admission |
| `OpenEmotion/emotiond/self_model_adapter.py` | 候删 | 当前 formal mainline 无 caller；先保留 compat/shim 分类，下一轮再做删除 admission |
| `OpenEmotion/emotiond/self_model_mirror.py` | 候删 | 当前 formal mainline 无 caller；先保留 reference-only 分类，下一轮再做删除 admission |
...
2. 再单开一轮删除 admission，验证以下路径在 formal mainline 上确实无 caller 后再删：
