...
|---|---|---|---|
| endogenous drive state | `OpenEmotion/openemotion/endogenous_drives/*` | `proto_self_v2` 只可读受治理 drive context | `EgoCore`, `emotiond/drives/*`, `drive_adapter.py`, `drive_homeostasis.py`, `homeostasis.py` |
| drive accumulation / decay / competition | `OpenEmotion/openemotion/endogenous_drives/*` | runtime mainline downstream consumers | `EgoCore`, legacy drive modules |
| priority snapshot / candidate bias | `OpenEmotion/openemotion/endogenous_drives/*` | governed prioritization path | transport / delivery surfaces |
| maintenance candidate generation | `OpenEmotion/openemotion/endogenous_drives/*` | host runtime may schedule only after Governor-compatible bridge | direct transport, direct tool execution |
...
- `OpenEmotion/openemotion/endogenous_drives/*` 是 `WP9` formal owner target
- `proto_self_v2` 可以消费 drive context，但不能拥有 drive state
- `emotiond/drives/*` 与 `drive_adapter.py` 只保留 bounded compatibility / migration / replay-friendly access semantics
...
- 不把 `emotiond/drives/*` 保留为长期 formal owner
