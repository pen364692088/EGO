...
def _write_markdown(path: Path, payload: Dict[str, Any]) -> None:
    ...
    drain = dict(payload.get("drain_result") or {})
    first_record = dict((drain.get("drained_records") or [None])[0] or {})
    lines = [
        "# MVP12 Proactive Outbox Drain",
        "",
        ...
        f"- drain_status: `{drain.get('status')}`",
        "",
        "## Simulated Send Record",
        "",
        f"- drained_records: `{len(drain.get('drained_records') or [])}`",
        f"- transport_source: `{first_record.get('transport_source')}`",
        f"- reply_authority: `{first_record.get('reply_authority')}`",
        f"- reply_origin: `{first_record.get('reply_origin')}`",
        f"- reply_text: `{first_record.get('reply_text')}`",
    ]
    path.write_text("\n".join(lines) + "\n", encoding="utf-8")


async def run_proactive_outbox_drain_session(
    *,
    messages: List[str],
    session_id: str,
    ...
    if runtime.proto_self_runtime is None:
        raise RuntimeError("Proto-Self runtime is not enabled; cannot run MVP12 proactive outbox drain")

    last_activity_at = state.get_chat_state().last_activity_at
    if last_activity_at is None:
        raise RuntimeError("No chat activity timestamp available for controlled proactive outbox drain")
    ...
    drain_result = drain_controlled_proactive_outbox(
        session_id=session_id,
        state=state,
        now_ts=scheduler_now + 3.0,
        drain_mode="simulated_send_record",
        transport_source="simulated_outbox_drain",
    )

    payload = {
        "schema_version": "mvp12.proactive_outbox_drain.v1",
        ...
        "drain_result": drain_result.to_dict(),
...
async def main() -> int:
    parser = argparse.ArgumentParser(description="Drain MVP12 proactive outbox into simulated send records without Telegram transport.")
    ...
    parser.add_argument(
        "--session-id",
        default=f"session:mvp12:proactive_outbox_drain:{datetime.now().strftime('%Y%m%d_%H%M%S')}",
    )
    parser.add_argument("--idle-seconds", type=float, default=900.0)
    parser.add_argument(
        "--output-json",
        default=str(ARTIFACTS_ROOT / "proactive_outbox_drain_current.json"),
    )
