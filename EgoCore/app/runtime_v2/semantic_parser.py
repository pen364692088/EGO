...
8. 如果运行时状态里有 recent_delivered_result_context / pending_result_continuation / active_task_summary / last_assistant_reply：
...
def build_parser_context(
    ...
    pending_result_continuation = dict(chat_prompt_context.get("pending_result_continuation") or {})
...
def decide_runtime_action(graph: ParsedIntentGraph, state: Any) -> str:
    ...
    # 状态查询优先
    if graph.has_status_query:
        pending_continuation = dict(getattr(state, "pending_result_continuation", None) or {})
...
def build_runtime_status_reply(state: Any, *, assume_active: bool = False) -> str:
    ...
    pending_continuation = dict(getattr(state, "pending_result_continuation", None) or {})
    if pending_continuation:
        target_name = str(pending_continuation.get("target_name") or "").strip()
        requested_mode = str(pending_continuation.get("requested_mode") or "analyze").strip() or "analyze"
        status = str(pending_continuation.get("status") or "pending").strip() or "pending"
