...
- **Proto-Self Kernel v1** 已完成真实 Telegram 主链接入，并完成 P4 真链收口
  - `tool:file` blocked / success 已同 family、不同 identity
  - 首次 retry-success 已点亮 `repair_closure=true`
- **MVS E5 观察状态**：已拿到 `/new continuity` 与 `restart continuity` 的强真实正证据
  - 显式默认规则在真实链路中已由 EgoCore `profile_memory` 持久化并被再次命中
  - 当前仍不能报 `E5 稳定成立` 或 `Developmental Self` 准入通过
  - `restore` 仍是当前最高优先级缺口
- **最新报告**
...
- **说明**
  - 下方 `cycle_core_v1` / `WS_C1` 表格保留为历史验证基线
  - 当前最新验收前沿应以 Proto-Self 主链、P4 报告与 `mvs_e5_observation` 观察文档为准
...
- 当前运行时 / replay / 验证主线仍是 **Proto-Self Kernel v1 实现线**
- 本轮没有把 V2 规格直接提升成当前 runtime contract

| 文档 | 说明 |
|------|------|
| [设计稿](docs/PROTO_SELF_KERNEL_V1_DESIGN.md) | V1 历史设计入口，不再是 canonical source |
| [接口草案](docs/PROTO_SELF_KERNEL_V1_SPEC.md) | V1 接口草案 / 历史实现桥接，不是 V2 canonical source |
| [V2 正式规格](docs/PROTO_SELF_KERNEL_V2_SPEC.md) | 下一代 Proto-Self 内核 canonical source，不是当前 runtime contract |
| [V2 迁移映射](docs/PROTO_SELF_KERNEL_V2_MIGRATION_MAP.md) | V1 -> V2 命名 / 字段 / replay 映射 |
| `openemotion/proto_self_v2/` | V2 bounded implementation path，显式 `proto_self.v2` 入口，V1 默认主线不变 |
...
| 阶段 | 优先级 | 状态 |
|------|--------|------|
| C3 观察期 | P0 | 观察已在推进；当前 O1 的最高优先级缺口是 `restore continuity` |
| v1.1.0 shim 清理 | P1 | 历史待办，需按最新 PROGRAM_STATE 重新排优先级 |
| 监控 shadow mode 稳定性 | P2 | 仍可继续，但不替代 Proto-Self 当前主链验收口径 |
