...
from .history import SocialRevisionRecord, compute_diff, hash_payload
...
DEFAULT_OWNER_ARTIFACTS_DIR = (
    Path(__file__).resolve().parents[2] / "artifacts" / "mvp17" / "formal_social_self"
)
...
class SocialSelfStore:
    ...
    def state_file(self, identity: Optional[str] = None) -> Path:
        return self._identity_dir(identity or self.default_identity) / "social_self_state.json"

    def revision_log_file(self, identity: Optional[str] = None) -> Path:
        return self._identity_dir(identity or self.default_identity) / "social_self_revisions.jsonl"

    def load(self, identity: Optional[str] = None) -> Optional[SocialSelfState]:
        path = self.state_file(identity)
        if not path.exists():
            return None
        return SocialSelfState.model_validate(json.loads(path.read_text(encoding="utf-8")))
    ...
    def load_revision_log(self, identity: Optional[str] = None) -> List[SocialRevisionRecord]:
        path = self.revision_log_file(identity)
        if not path.exists():
            return []
        return [
            SocialRevisionRecord.model_validate(json.loads(line))
    ...
    def save(
        ...
        *,
        ...
        if state.identity_handle != resolved_identity:
            raise ValueError(f"social_state_identity_mismatch:{state.identity_handle}:{resolved_identity}")
        before_snapshot = self.load_snapshot(resolved_identity) or {}
        verdict = validate_social_state(state)
        if not verdict.accepted:
            raise ValueError(f"social_state_governance_rejected: {verdict.violations}")

        revisions = self.load_revision_log(resolved_identity)
        model_version = len(revisions) + 1
        revision_id = f"social_rev_{model_version:06d}"
        timestamp = _utc_now_iso()
        trace_ref = trace_reference or f"social_self:{update_source}"

        after_state = SocialSelfState.model_validate(state.model_dump(mode="json"))
    ...
    def replay(self, identity: Optional[str] = None) -> Optional[SocialSelfState]:
