...
from .history import DriveRevisionMarker, DriveRevisionRecord, compute_diff, hash_payload
from .replay import replay_state_from_revisions
from .state import DriveState

DEFAULT_OWNER_ARTIFACTS_DIR = (
    Path(__file__).resolve().parents[2] / "artifacts" / "mvp14" / "formal_endogenous_drives"
)
...
class EndogenousDriveStore:
    ...
    def state_file(self, identity: Optional[str] = None) -> Path:
        return self._identity_dir(identity or self.default_identity) / "drive_state.json"

    def revision_log_file(self, identity: Optional[str] = None) -> Path:
        return self._identity_dir(identity or self.default_identity) / "drive_revisions.jsonl"

    def load(self, identity: Optional[str] = None) -> Optional[DriveState]:
        path = self.state_file(identity)
        if not path.exists():
            return None
        return DriveState.model_validate(json.loads(path.read_text(encoding="utf-8")))
    ...
    def load_revision_log(self, identity: Optional[str] = None) -> List[DriveRevisionRecord]:
        path = self.revision_log_file(identity)
        if not path.exists():
            return []
        return [
            DriveRevisionRecord.model_validate(json.loads(line))
    ...
    def save(
        ...
        *,
        ...
        if not verdict.accepted:
            raise ValueError(f"drive_state_governance_rejected: {verdict.violations}")
        revisions = self.load_revision_log(resolved_identity)
        model_version = len(revisions) + 1
        revision_id = f"drive_rev_{model_version:06d}"
        timestamp = _utc_now_iso()
        trace_ref = trace_reference or f"endogenous_drives:{update_source}"

        after_state = DriveState.model_validate(state.model_dump(mode="json"))
        ...
        after_state.drive_history.record_revision_marker(marker)
        after_snapshot = after_state.model_dump(mode="json")
        record = DriveRevisionRecord(
    ...
    def replay(self, identity: Optional[str] = None) -> Optional[DriveState]:
