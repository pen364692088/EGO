...
SCENARIO_BANK_DIR = ROOT / "OpenEmotion" / "scenarios" / "mvp14_observation_bank"
SCENARIO_MANIFEST_SCHEMA_VERSION = "mvp14.observation_scenario.v1"
...
def validate_scenario_manifest(manifest: Dict[str, Any]) -> List[str]:
    ...
    optional_mapping_fields = ("resource_budget_hint", "maintenance_context", "drive_overrides")
    for field_name in optional_mapping_fields:
        value = manifest.get(field_name)
        if value is not None and not isinstance(value, dict):
            errors.append(field_name)

    for field_name in ("maintenance_debts", "homeostatic_updates"):
        value = manifest.get(field_name)
...
def load_scenario_manifest(path: str | Path) -> Dict[str, Any]:
    ...
    if errors:
        raise ValueError(
            f"invalid MVP14 scenario manifest {manifest_path}: {', '.join(errors)}"
        )
    return {
        ...
        "drive_overrides": dict(payload.get("drive_overrides") or {}),
        "maintenance_debts": list(payload.get("maintenance_debts") or []),
        "homeostatic_updates": list(payload.get("homeostatic_updates") or []),
