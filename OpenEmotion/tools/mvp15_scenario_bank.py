...
from openemotion.reflective_self import ReflectionTargetType


ROOT = Path(__file__).resolve().parents[2]
SCENARIO_BANK_DIR = ROOT / "OpenEmotion" / "scenarios" / "mvp15_observation_bank"
SCENARIO_MANIFEST_SCHEMA_VERSION = "mvp15.observation_scenario.v1"
...
def _validate_owner_bootstrap(owner_bootstrap: Any) -> List[str]:
    if owner_bootstrap is None:
        return []
    if not isinstance(owner_bootstrap, dict):
        return ["owner_bootstrap"]

    errors: List[str] = []
    target_type = owner_bootstrap.get("target_type")
    if target_type not in (None, ""):
        try:
            ReflectionTargetType(_trim(target_type))
        except ValueError:
            errors.append("owner_bootstrap.target_type")

    extra_targets = owner_bootstrap.get("extra_targets")
    if extra_targets is not None and not isinstance(extra_targets, list):
        errors.append("owner_bootstrap.extra_targets")
    return errors
...
def validate_scenario_manifest(manifest: Dict[str, Any]) -> List[str]:
    ...
    for field_name in ("resource_budget_hint", "maintenance_context", "owner_bootstrap"):
        value = manifest.get(field_name)
        if value is not None and not isinstance(value, dict):
            errors.append(field_name)
    errors.extend(_validate_owner_bootstrap(manifest.get("owner_bootstrap")))
...
def load_scenario_manifest(path: str | Path) -> Dict[str, Any]:
    ...
    if errors:
        raise ValueError(f"invalid MVP15 scenario manifest {manifest_path}: {', '.join(errors)}")
    return {
