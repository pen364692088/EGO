...
from mvp19_scenario_bank import (
    SCENARIO_BANK_DIR,
...
from run_mvp19_controlled_observation import ARTIFACTS_ROOT, run_controlled_observation
...
def _render_markdown(payload: Dict[str, Any]) -> str:
    lines = [
        "# MVP19 Controlled Observation Batch Report",
        "",
        ...
        f"- self_integration_proposal_present_count: `{payload.get('self_integration_proposal_present_count')}`",
        ...
        f"- distinct_dominant_axes: `{payload.get('distinct_dominant_axes')}`",
    ...
    for scenario in payload.get("reports") or []:
        lines.append(
            ...
            f"proposal=`{scenario.get('self_integration_proposal_present')}` "
...
async def run_controlled_observation_batch(
    *,
    ...
    if not scenarios:
        raise RuntimeError("no valid MVP19 observation scenarios found")
    ...
    for index, scenario in enumerate(scenarios, start=1):
        ...
        payload = await run_controlled_observation(
            ...
            session_id=f"session:mvp19:batch:{index:03d}:{scenario['scenario_id']}",
            output_json=None,
        ...
        reports.append(
            {
                ...
                "self_integration_proposal_present": bool(payload.get("self_integration_proposal_present")),
                ...
                "dominant_pressure_axis": payload.get("dominant_pressure_axis"),
                "surface_reasons": list((payload.get("self_integration_delta") or {}).get("surface_reasons") or []),
                "report_json": str(scenario_dir / "mvp19_controlled_observation_report.json"),
                "report_md": str(scenario_dir / "mvp19_controlled_observation_report.md"),
            }
        )
    ...
    self_integration_proposal_present_count = sum(
        1 for report in reports if report["self_integration_proposal_present"]
    )
    ...
    distinct_dominant_axes = sorted(
        {
            str(report["dominant_pressure_axis"])
            for report in reports
            if str(report["dominant_pressure_axis"] or "").strip()
        }
    )
    ...
    e5_pass = (
        ...
        and self_integration_proposal_present_count == report_count
    ...
    payload = {
        "schema_version": "mvp19.controlled_observation_batch.v1",
        ...
        "self_integration_proposal_present_count": self_integration_proposal_present_count,
        ...
        "distinct_dominant_axes": distinct_dominant_axes,
        ...
        "boundary": (
            "This report proves repeated controlled runtime-mainline selfhood integration "
            "proposal-only writeback on the WP14 controlled axis. It does not prove live autonomy, "
            "direct reply authority, or broader transport maturity."
        ),
        "external_risk_note": (
    ...
    report_json = batch_dir / "mvp19_controlled_observation_batch_report.json"
    report_md = batch_dir / "mvp19_controlled_observation_batch_report.md"
...
async def main() -> int:
    parser = argparse.ArgumentParser(description="Run a controlled MVP19 observation batch from the scenario bank.")
    ...
    parser.add_argument(
        "--output-json",
        default=str(ARTIFACTS_ROOT / "mvp19_controlled_observation_batch_current.json"),
    )
