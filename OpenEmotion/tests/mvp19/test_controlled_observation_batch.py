...
from mvp19_scenario_bank import (  # noqa: E402
...
def test_load_scenario_manifest_rejects_unsupported_source_class(tmp_path):
    ...
    _write_manifest(
        manifest_path,
        {
            ...
            "dialogue_frame_target": "low_confidence_embodied_growth_conflict",
...
async def test_batch_controlled_observation_runs_and_aggregates(tmp_path, monkeypatch):
    from run_mvp19_controlled_observation_batch import run_controlled_observation_batch
    ...
    _write_manifest(
        bank_dir / "repo_b.json",
        {
            **base_payload,
            ...
            "dialogue_frame_target": "high_maintenance_reflective_review",
        },
    )
    _write_manifest(
        bank_dir / "repo_c.json",
        {
            **base_payload,
            ...
            "dialogue_frame_target": "social_repair_boundary_guard_conflict",
        },
    )
    ...
    async def _fake_run_controlled_observation(**kwargs):
        ...
        return {
            ...
            "selected_priority": "review" if scenario["scenario_id"] == "repo_case_b" else "conserve",
            "dominant_pressure_axis": "embodied_self",
            "self_integration_delta": {
                "surface_reasons": ["maintenance_pressure", "embodied_pressure", "social_repair"]
            },
            "selfhood_integration_writeback": {
                "decision": {"gate_verdict": "allow_writeback"},
                "record": {"revision_id": f"integration_rev_{scenario['scenario_id']}"},
            },
        }

    monkeypatch.setattr(
        "run_mvp19_controlled_observation_batch.run_controlled_observation",
    ...
    assert payload["self_integration_proposal_present_count"] == 3
