...
def _write_markdown(path: Path, payload: Dict[str, Any]) -> None:
    ...
    emitted = dict(delivery.get("emitted_delivery") or {})
    lines = [
        "# MVP12 Controlled Proactive Delivery",
        "",
        ...
        f"- delivery_reason: `{delivery.get('reason')}`",
        "",
        "## Emitted Delivery",
        "",
        f"- emitted: `{bool(emitted)}`",
        f"- delivery_mode: `{emitted.get('delivery_mode')}`",
        f"- transport_source: `{emitted.get('transport_source')}`",
        f"- reply_authority: `{emitted.get('reply_authority')}`",
        f"- reply_origin: `{emitted.get('reply_origin')}`",
        f"- reply_text: `{emitted.get('reply_text')}`",
    ]
    path.write_text("\n".join(lines) + "\n", encoding="utf-8")


async def run_controlled_delivery_session(
    *,
    messages: List[str],
    session_id: str,
    ...
    if runtime.proto_self_runtime is None:
        raise RuntimeError("Proto-Self runtime is not enabled; cannot run MVP12 controlled delivery")

    last_activity_at = state.get_chat_state().last_activity_at
    if last_activity_at is None:
        raise RuntimeError("No chat activity timestamp available for controlled delivery")
    ...
    payload = {
        "schema_version": "mvp12.controlled_proactive_delivery.v1",
...
async def main() -> int:
    parser = argparse.ArgumentParser(description="Consume MVP12 pending proactive followup into a controlled delivery artifact.")
    ...
    parser.add_argument(
        "--session-id",
        default=f"session:mvp12:controlled_delivery:{datetime.now().strftime('%Y%m%d_%H%M%S')}",
    )
    parser.add_argument("--idle-seconds", type=float, default=900.0)
    parser.add_argument(
        "--output-json",
        default=str(ARTIFACTS_ROOT / "controlled_proactive_delivery_current.json"),
    )
