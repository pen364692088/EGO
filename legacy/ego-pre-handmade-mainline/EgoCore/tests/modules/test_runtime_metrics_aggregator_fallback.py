...
class TestFallbackScenarios:
    ...
    def test_fallback_negative_counter(self):
        """Fallback: 负值 counter"""
        # 这个在 core 层会报错，但 adapter fallback 应该捕获
        result = self.adapter.record_with_fallback(
            metric_name="test",
            metric_type="counter",
            value=-1.0
    ...
    def test_no_error_propagation(self):
        ...
        try:
            result = self.adapter.record_with_fallback(
                metric_name="",
                metric_type="invalid",
                value=None  # 这会导致 TypeError
            )
            # 如果走到这里，说明 fallback 捕获了 TypeError
            assert result["success"] is True
            assert result["metric_id"] == "dropped"
...
class TestFallbackBehavior:
    ...
    def test_fallback_no_error_field(self):
        ...
        # fallback 结果不应该有 error 字段，让调用方无感知
