...
SCENARIO_BANK_DIR = ROOT / "OpenEmotion" / "scenarios" / "mvp20_observation_bank"
SCENARIO_MANIFEST_SCHEMA_VERSION = "mvp20.observation_scenario.v1"
...
def load_scenario_manifest(path: str | Path) -> Dict[str, Any]:
    ...
    if errors:
        raise ValueError(f"invalid MVP20 scenario manifest {manifest_path}: {', '.join(errors)}")
    return {
        ...
        "initiative_context": dict(payload.get("initiative_context") or {}),
