...
SCENARIO_BANK_DIR = ROOT / "OpenEmotion" / "scenarios" / "mvp16_observation_bank"
SCENARIO_MANIFEST_SCHEMA_VERSION = "mvp16.observation_scenario.v1"
...
def load_scenario_manifest(path: str | Path) -> Dict[str, Any]:
    ...
    if errors:
        raise ValueError(f"invalid MVP16 scenario manifest {manifest_path}: {', '.join(errors)}")
    return {
        ...
        "developmental_context": dict(payload.get("developmental_context") or {}),
