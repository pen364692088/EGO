...
SCENARIO_BANK_DIR = ROOT / "OpenEmotion" / "scenarios" / "mvp21_observation_bank"
SCENARIO_MANIFEST_SCHEMA_VERSION = "mvp21.observation_scenario.v1"
...
def load_scenario_manifest(path: str | Path) -> Dict[str, Any]:
    ...
    if errors:
        raise ValueError(f"invalid MVP21 scenario manifest {manifest_path}: {', '.join(errors)}")
    return {
        ...
        "host_proactive_context": dict(payload.get("host_proactive_context") or {}),
