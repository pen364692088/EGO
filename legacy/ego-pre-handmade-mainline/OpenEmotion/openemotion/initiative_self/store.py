...
from .history import InitiativeRevisionRecord, compute_diff, hash_payload
from .replay import replay_state_from_revisions
from .schemas import InitiativeLedgerEntry
from .state import InitiativeSelfState, REQUIRED_WRITEBACK_GATE

DEFAULT_OWNER_ARTIFACTS_DIR = (
    Path(__file__).resolve().parents[2] / "artifacts" / "mvp20" / "formal_initiative_self"
)
...
class InitiativeSelfStore:
    ...
    def state_file(self, identity: Optional[str] = None) -> Path:
        return self._identity_dir(identity or self.default_identity) / "initiative_self_state.json"

    def revision_log_file(self, identity: Optional[str] = None) -> Path:
        return self._identity_dir(identity or self.default_identity) / "initiative_self_revisions.jsonl"

    def load(self, identity: Optional[str] = None) -> Optional[InitiativeSelfState]:
        path = self.state_file(identity)
        if not path.exists():
            return None
        return InitiativeSelfState.model_validate(json.loads(path.read_text(encoding="utf-8")))
    ...
    def load_revision_log(self, identity: Optional[str] = None) -> List[InitiativeRevisionRecord]:
        path = self.revision_log_file(identity)
        if not path.exists():
            return []
        return [
            InitiativeRevisionRecord.model_validate(json.loads(line))
    ...
    def save(
        ...
        *,
        ...
        if state.identity_handle != resolved_identity:
            raise ValueError(f"initiative_state_identity_mismatch:{state.identity_handle}:{resolved_identity}")
        before_snapshot = self.load_snapshot(resolved_identity) or {}
        verdict = validate_initiative_state(state)
        if not verdict.accepted:
            raise ValueError(f"initiative_state_governance_rejected: {verdict.violations}")

        revisions = self.load_revision_log(resolved_identity)
        model_version = len(revisions) + 1
        revision_id = f"initiative_rev_{model_version:06d}"
        timestamp = _utc_now_iso()
        trace_ref = trace_reference or f"initiative_self:{update_source}"

        after_state = InitiativeSelfState.model_validate(state.model_dump(mode="json"))
        after_state.owner_revision = model_version
        after_state.last_revision_id = revision_id
        ledger_id = f"persist_{revision_id}"
        after_state.initiative_ledger[ledger_id] = InitiativeLedgerEntry(
            ledger_id=ledger_id,
        ...
        record = InitiativeRevisionRecord(
    ...
    def replay(self, identity: Optional[str] = None) -> Optional[InitiativeSelfState]:
