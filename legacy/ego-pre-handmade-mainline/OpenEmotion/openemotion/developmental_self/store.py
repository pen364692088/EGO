...
DEFAULT_OWNER_ARTIFACTS_DIR = (
    Path(__file__).resolve().parents[2] / "artifacts" / "mvp16" / "formal_developmental_self"
)
...
class DevelopmentalSelfStore:
    ...
    def state_file(self, identity: Optional[str] = None) -> Path:
        return self._identity_dir(identity or self.default_identity) / "developmental_self_state.json"

    def revision_log_file(self, identity: Optional[str] = None) -> Path:
        return self._identity_dir(identity or self.default_identity) / "developmental_self_revisions.jsonl"

    def load(self, identity: Optional[str] = None) -> Optional[DevelopmentalSelfState]:
        path = self.state_file(identity)
        if not path.exists():
            return None
        return DevelopmentalSelfState.model_validate(json.loads(path.read_text(encoding="utf-8")))
    ...
    def load_revision_log(self, identity: Optional[str] = None) -> List[DevelopmentalRevisionRecord]:
        path = self.revision_log_file(identity)
        if not path.exists():
            return []
        return [
            DevelopmentalRevisionRecord.model_validate(json.loads(line))
    ...
    def save(
        ...
        *,
        ...
        if state.identity_handle != resolved_identity:
            raise ValueError(
                f"developmental_state_identity_mismatch:{state.identity_handle}:{resolved_identity}"
            )
        before_snapshot = self.load_snapshot(resolved_identity) or {}
        verdict = validate_developmental_state(state)
        if not verdict.accepted:
            raise ValueError(f"developmental_state_governance_rejected: {verdict.violations}")

        revisions = self.load_revision_log(resolved_identity)
        model_version = len(revisions) + 1
        revision_id = f"developmental_rev_{model_version:06d}"
        timestamp = _utc_now_iso()
        trace_ref = trace_reference or f"developmental_self:{update_source}"

        after_state = DevelopmentalSelfState.model_validate(state.model_dump(mode="json"))
        ...
        record = DevelopmentalRevisionRecord(
    ...
    def replay(self, identity: Optional[str] = None) -> Optional[DevelopmentalSelfState]:
