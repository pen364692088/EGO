...
from .history import DevelopmentalRevisionRecord, hash_payload
from .state import DevelopmentalSelfState


class DevelopmentalReplayError(RuntimeError):
    pass


def replay_state_from_revisions(
    revisions: Iterable[DevelopmentalRevisionRecord],
    *,
    base_snapshot: Optional[dict] = None,
) -> Optional[DevelopmentalSelfState]:
    ...
    for record in revisions:
        if record.previous_state_hash != current_hash:
            raise DevelopmentalReplayError(
    ...
    if final_snapshot is None:
        return None
    return DevelopmentalSelfState.model_validate(final_snapshot)
