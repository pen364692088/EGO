...
from .schemas import IntegrationLedgerEntry
from .state import REQUIRED_WRITEBACK_GATE, SelfhoodIntegrationState

DEFAULT_OWNER_ARTIFACTS_DIR = (
    Path(__file__).resolve().parents[2]
    / "artifacts"
    / "mvp19"
    / "formal_selfhood_integration"
)
...
class SelfhoodIntegrationStore:
    def __init__(
        self,
        base_dir: Optional[str | Path] = None,
        *,
        default_identity: str = "openemotion",
    ):
    ...
    def state_file(self, identity: Optional[str] = None) -> Path:
        return self._identity_dir(identity or self.default_identity) / "selfhood_integration_state.json"

    def revision_log_file(self, identity: Optional[str] = None) -> Path:
        return self._identity_dir(identity or self.default_identity) / "selfhood_integration_revisions.jsonl"

    def load(self, identity: Optional[str] = None) -> Optional[SelfhoodIntegrationState]:
        path = self.state_file(identity)
        if not path.exists():
            return None
        return SelfhoodIntegrationState.model_validate(json.loads(path.read_text(encoding="utf-8")))
    ...
    ) -> List[SelfhoodIntegrationRevisionRecord]:
        path = self.revision_log_file(identity)
        if not path.exists():
            return []
        return [
            SelfhoodIntegrationRevisionRecord.model_validate(json.loads(line))
    ...
    def save(
        ...
        *,
        ...
        if state.identity_handle != resolved_identity:
            raise ValueError(
                f"selfhood_integration_identity_mismatch:{state.identity_handle}:{resolved_identity}"
            )
        before_snapshot = self.load_snapshot(resolved_identity) or {}
        verdict = validate_selfhood_integration_state(state)
        if not verdict.accepted:
            raise ValueError(f"selfhood_integration_governance_rejected: {verdict.violations}")

        revisions = self.load_revision_log(resolved_identity)
        model_version = len(revisions) + 1
        revision_id = f"integration_rev_{model_version:06d}"
        timestamp = _utc_now_iso()
        trace_ref = trace_reference or f"selfhood_integration:{update_source}"

        after_state = SelfhoodIntegrationState.model_validate(state.model_dump(mode="json"))
        after_state.owner_revision = model_version
        after_state.last_revision_id = revision_id
        ledger_id = f"persist_{revision_id}"
        after_state.integration_ledger[ledger_id] = IntegrationLedgerEntry(
            ledger_id=ledger_id,
        ...
        record = SelfhoodIntegrationRevisionRecord(
    ...
    def replay(self, identity: Optional[str] = None) -> Optional[SelfhoodIntegrationState]:
