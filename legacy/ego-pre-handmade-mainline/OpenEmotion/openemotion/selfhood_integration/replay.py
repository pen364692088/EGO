...
from .history import SelfhoodIntegrationRevisionRecord, hash_payload
from .state import SelfhoodIntegrationState


class SelfhoodIntegrationReplayError(RuntimeError):
    pass


def replay_state_from_revisions(
    revisions: Iterable[SelfhoodIntegrationRevisionRecord],
    *,
    base_snapshot: Optional[dict] = None,
) -> Optional[SelfhoodIntegrationState]:
    ...
    for record in revisions:
        if record.previous_state_hash != current_hash:
            raise SelfhoodIntegrationReplayError(
    ...
    if final_snapshot is None:
        return None
    return SelfhoodIntegrationState.model_validate(final_snapshot)
