...
from run_mvp18_controlled_observation_batch import (  # noqa: E402
    SCENARIO_MANIFEST_SCHEMA_VERSION,
    load_scenario_bank,
    load_scenario_manifest,
    run_controlled_observation_batch,
...
def test_load_scenario_manifest_rejects_unsupported_source_class(tmp_path):
    ...
    _write_manifest(
        manifest_path,
        {
            ...
            "dialogue_frame_target": "high_resource_pressure_conserve",
...
async def test_batch_controlled_observation_runs_and_aggregates(tmp_path, monkeypatch):
    ...
    _write_manifest(
        bank_dir / "repo_b.json",
        {
            **base_payload,
            ...
            "dialogue_frame_target": "consequence_memory_repair_review",
        },
    )
    _write_manifest(
        bank_dir / "repo_c.json",
        {
            **base_payload,
            ...
            "dialogue_frame_target": "boundary_guard_repair_only",
        },
    )
    ...
    async def _fake_run_controlled_observation(**kwargs):
        ...
        return {
            ...
            "resource_boundary_snapshot": {
                "action_ref": f"delivery:{scenario['scenario_id']}",
            },
            "embodied_self_delta": {"surface_reasons": ["resource_pressure", "boundary_pressure"]},
            "embodied_writeback": {
                "decision": {"gate_verdict": "allow_writeback"},
                "record": {"revision_id": f"embodied_rev_{scenario['scenario_id']}"},
            },
        }

    monkeypatch.setattr(
        "run_mvp18_controlled_observation_batch.run_controlled_observation",
    ...
    assert payload["embodied_proposal_present_count"] == 3
