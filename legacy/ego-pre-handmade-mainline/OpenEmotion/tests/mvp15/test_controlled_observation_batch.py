...
from mvp15_scenario_bank import (  # noqa: E402
    SCENARIO_MANIFEST_SCHEMA_VERSION,
    load_scenario_bank,
    load_scenario_manifest,
    validate_scenario_manifest,
)
from run_mvp15_controlled_observation_batch import run_controlled_observation_batch  # noqa: E402
...
def test_load_scenario_manifest_rejects_unsupported_source_class(tmp_path):
    ...
    _write_manifest(
        manifest_path,
        {
            ...
            "dialogue_frame_target": "decision_revision_under_reflective_pressure",
...
async def test_batch_controlled_observation_runs_and_aggregates(tmp_path, monkeypatch):
    ...
    _write_manifest(
        bank_dir / "repo_b.json",
        {
            **base_payload,
            ...
            "dialogue_frame_target": "trajectory_counterfactual_review",
        },
    )
    _write_manifest(
        bank_dir / "repo_c.json",
        {
            **base_payload,
            ...
            "dialogue_frame_target": "maintenance_followup_with_guarded_reflection",
        },
    )

    async def _fake_run_controlled_observation(**kwargs):
        scenario = dict(kwargs.get("scenario_manifest") or {})
        target_id = f"target:{scenario['scenario_id']}"
        return {
            "status": "pass",
            "replay_valid": True,
            "reflection_candidate_present": True,
            "proposal_discipline_consistent": True,
            "behavioral_authority_none": True,
            "target_ids": [target_id],
            "reflective_self_delta": {"surface_reasons": ["reflection_pressure", "unresolved_items"]},
            "reflective_self_writeback": {
                "decision": {"gate_verdict": "allow_writeback"},
                "record": {"revision_id": f"reflective_rev_{scenario['scenario_id']}"},
            },
        }

    monkeypatch.setattr(
        "run_mvp15_controlled_observation_batch.run_controlled_observation",
    ...
    assert payload["reflection_candidate_present_count"] == 3
    assert payload["proposal_discipline_consistent_count"] == 3
