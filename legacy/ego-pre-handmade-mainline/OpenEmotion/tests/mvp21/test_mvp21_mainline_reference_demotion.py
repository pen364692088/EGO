from tools.verify_mvp21_mainline_wiring import inspect_wiring


def test_mvp21_host_proactive_substrate_remains_host_substrate_only():
...
def test_mvp21_roadmap_materials_remain_reference_only():
    report = inspect_wiring()
...
def test_mvp21_current_runtime_consumer_is_formal_owner_path():
    report = inspect_wiring()

    assert report["formal_owner"]["initiative_realization_package_present"] is True
    assert (
        report["current_runtime_mainline"][
    ...
    assert (
        report["status"]
        == "current_runtime_initiative_realization_consumer_present_legacy_reference_only"
    )
