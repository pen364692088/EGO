...
EGO 是 AI Agent 项目的总仓。当前默认 human/operator 体验主线已经切到 `Ego_handmade`；旧 `EgoCore`、`OpenEmotion`、`ego_desktop_lab` 已降为 legacy reference / fallback / algorithm source。
...
- `Ego_handmade` 是默认 operator-first implementation lane：自然语言先由 LLM 理解，再进入 runtime gate / approval / memory / trace。
...
- 本阶段只记录 `Ego_handmade-first repo transition / legacy demotion recorded`，不声明 live autonomy、runtime efficacy、稳定用户收益或 consciousness。
- `Ego_handmade` 当前已具备 local candidate 级 operator runtime：memory、permission gate、operator comparison、real-use gate、human operator trial scaffold、runtime contract。
...
- 当前 repo 处于 `Ego_handmade-first transition / legacy demotion recorded`
...
- [Ego_handmade/agent_base.py](Ego_handmade/agent_base.py)
